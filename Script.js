...
/* ── Terminal interactif ───────────────────────────────────── */
(function () {
  let termHistory   = [];   // historique des commandes
  let historyIndex  = -1;
  let termReady     = false;
  ...
  function openTerminal() {
    ...
    document.body.style.overflow = 'hidden';
    if (!termReady) { showWelcome(); termReady = true; }
    setTimeout(() => { const inp = $('termInput'); if (inp) inp.focus(); }, 360);
  }
  ...
  const CMDS = {
    ...
    'cd projets'() {
      ...
      setTimeout(() => { window.location.href = 'Projets.html'; }, 700);
    },

    'cd parcours'() {
      appendLine('  → Navigation vers Parcours...', 'success');
      setTimeout(() => { window.location.href = 'Parcours.html'; }, 700);
    },
    ...
    'cd /'() { appendLine('/home/aymen/portfolio', 'success'); },
    'cd ~'()  { appendLine('/home/aymen/portfolio', 'success'); },
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!input) return;  // page sans terminal

    // Touche Entrée → exécuter commande
    ...
    // Clic sur point rouge = fermer
    if (dotR) dotR.addEventListener('click', closeTerminal);

    // Clic n'importe où dans le body = focus input
    if (body) body.addEventListener('click', () => input.focus());
  });

})();

/* ── Contact form (index.html & contact dans projets) ──────── */
function sendMail() {
  ...
  window.location.href = `mailto:aymen68212@gmail.com?subject=${subject}&body=${body}`;
}